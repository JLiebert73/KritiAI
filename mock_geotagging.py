...
def render_geotagging_page():
    ...
    tab1, tab2 = st.tabs(["🌍 Spatial Intelligence & Geotagging", "🧠 The KritiAI Core Brain (Moat)"])
    ...
    with tab2:
        st.markdown("### 🧠 The Multi-Modal Identity Resolution Engine")
        st.markdown("<p style='color:#a0a0a0; font-size: 0.95rem;'>This is the core MOAT of the KritiAI platform. We do not just extract polygons; we mathematically fuse disparate, fuzzy data points (handwritten leases, API pings, satellite signatures) into verified Digital Identities using high-dimensional vector space.</p>", unsafe_allow_html=True)
        
        # 1. Telemetry Dashboard
        st.markdown("#### Live System Telemetry")
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            st.metric("Active Graph Nodes", "1,402,891", "+142/sec")
        with c2:
            st.metric("Edges Resolved", "8,204,512", "+3,102/sec")
        with c3:
            st.metric("Vector Search Latency", "12ms", "-2ms")
        with c4:
            st.metric("Core Engine", "Prithvi-EO-7B", "Online")
